...
function updateStatusText(text)
{
    if (text != null)
    {
        text = '<i class="fas fa-check-double" style="margin-right: 3px;"></i> ' + text;
    }
...
function assignLiOnClick()
{
    ...
    $(".fileManager .fileIconLi.owned-image .filename").click(function (e) {
        liElement = $(this).parents('.fileIconLi');
        fileId = liElement.attr('dtfullurl');
        window.open(fileId, '_blank'); ;
    });
    
    $(".fileManager .fileIconLi.not-owned-image").click(function (e) {
        fileId = $(this).attr('dtfullurl');
        window.open(fileId, '_blank'); ;
    });
    

    
    $(".fileManager .fileIconLi.owned-image, .fileManager .fileIconLi.owned-folder").click(function (e) {
        e.stopPropagation();
         if (!$(e.target).is('.filename')) {
...
function showLoaderModal(timer)
{
    ...
    var pleaseWaitDiv = $('<div class="modal custom-width loader-modal" id="loaderModal" data-keyboard="false"><div class="modal-dialog modal-dialog-center" style="width: 300px;"><div class="lds-default"><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div></div></div></div></div>');
    pleaseWaitDiv.modal();
...
function moveItems() {
    if (countSelected() == 0) {
        alert('Please select some items to move.');
        return false;
    }
